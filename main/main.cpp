...
// Arduino.h is auto-included by the Arduino IDE for .ino files; under
// ESP-IDF + arduino-as-component we have to include it explicitly so
// Serial / String / pin macros / etc. resolve.
#include <Arduino.h>
...
// ---------------------------------------------------------------------------
// Tokenizer + detokenizer live in host_common now (this file's version was
// the source of truth; box adopted it too). See
// components/ti-emulator/components/host_common/src/ti_host_tokenizer.cpp.
// initTokenNames() must be called once from setup() before detokenizeLine.
// ---------------------------------------------------------------------------
#include "ti_host.h"
using tihost::tokenizeLine;
using tihost::detokenizeLine;
using tihost::initTokenNames;
...
// tokenizeLine + detokenizeLine forward decls no longer needed —
// declarations come from ti_host.h via the `using` block above.
...
// the prompt (cmdChainLoad("DSK1.GAME") in processInput) and via the
// interpreter's TP_RUN_SPEC dispatch when the statement appears inside
// a running program (e.g. `200 RUN "DSK1.GAME"` in a launcher menu).
// cmdOld swaps the in-memory program for the named one (FLASH./SDCARD./
// DSK<n>. prefix or bare-name FLASH default); em.run() then resets vars
// + starts executing from line 1.
